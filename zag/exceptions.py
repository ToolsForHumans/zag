...
def raise_with_cause(exc_cls, message, *args, **kwargs):
    ...
    *cause* (or extract a *cause* from the current stack if able) so that the
    ...
    class has a :py:meth:`~zag.exceptions.ZagException.pformat`
    ...
    that the :py:meth:`~zag.exceptions.ZagException.pformat` method
    shows them).

    :param exc_cls: the :py:class:`~zag.exceptions.ZagException`
                    class to raise.
    ...
    :param kwargs: any additional keyword arguments to pass to the
                   exceptions constructor.
    """
    if not issubclass(exc_cls, ZagException):
        raise ValueError("Subclass of zag exception is required")
    excutils.raise_with_cause(exc_cls, message, *args, **kwargs)


class ZagException(Exception):
    ...
    def __init__(self, message, cause=None):
        super(ZagException, self).__init__(message)
        self._cause = cause
    ...
    def pformat(self, indent=2, indent_text=" ", show_root_class=False):
        """Pretty formats a zag exception + any connected causes."""
        if indent < 0:
        ...
        while next_up is not None and next_up not in seen:
            ...
            if isinstance(next_up, ZagException):
                ...
                for i, line in enumerate(lines):
                    ...
                    if i + 1 != len(lines):
                        buf.write(os.linesep)
            if not isinstance(next_up, ZagException):
                # Don't go deeper into non-zag exceptions... as we
...
class ExecutionFailure(ZagException):
...
class DependencyFailure(ZagException):
...
class MissingDependencies(DependencyFailure):
    ...
    :py:class:`~zag.exceptions.ZagException`.
    """
...
class WrappedFailure(mixins.StrMixin, Exception):
    ...
    *may* reraise this exception type to allow users to handle the contained
    ...
    :param causes: the :py:class:`~zag.types.failure.Failure` objects
