...
class ZkBackend(path_based.PathBasedBackend):
    ...
    by :py:func:`~zag.utils.kazoo_utils.make_client` and the transfer
    ...
    :py:func:`~zag.utils.kazoo_utils.make_client` such
    that if a client was not provided by the caller one will be created
    according to :py:func:`~zag.utils.kazoo_utils.make_client`'s
    specification
    """
...
class ZkConnection(path_based.PathBasedConnection):
    ...
    def _exc_wrapper(self):
        ...
        then group them into corresponding zag exceptions (not doing
