...
class Retry(atom.Atom):
    ...
    object is an atom it may also provide :meth:`~zag.retry.Retry.execute`
    and :meth:`~zag.retry.Retry.revert` methods to alter the inputs of
    connected atoms (depending on the desired strategy to be used this can be
    quite useful).

    NOTE(harlowja): the :meth:`~zag.retry.Retry.execute` and
    :meth:`~zag.retry.Retry.revert` and
    :meth:`~zag.retry.Retry.on_failure` will automatically be given
...
class Times(Retry):
    ...
    :py:class:`~zag.atom.Atom` constructor.
    """
...
class ForEach(ForEachBase):
    ...
    :param values: values collection to iterate over and provide to
                   ...
                   :py:meth:`~zag.atom.Atom.execute` method, which
